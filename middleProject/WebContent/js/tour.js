...
function getListItem(index, places) {
	...
	$(el).on('click', function() {
		...
		// 클릭한 장소를 지도에서 강조 표시하는 등의 원하는 동작을 추가
	});
						...
						var listItem = $('<li class="lists"></li>')
							.html(`<table>
									...
									<div class="adminPlace" style="display:none" name="${item.tour_no}">
										...
										<input type="button" class="cateBtn" name="tourUpdate" value="수정">
									</div>
								</table>
								<hr>`);
								...
								.html(`<table>
        									<tr>
        										<td rowspan="2">
        											<div class="tour_img_div">
        												<img src="${path}/images/tour/${item.tour_img}"
        													style="width: 100px; height: 100px; border-radius: 70%; margin-right:10px;">
        											</div>
        										</td>
											...
											<div class="adminPlace" style="display:none" name="${item.tour_no}">
												<input type="button" class="cateBtn" name="tourDelete" value="삭제">
												<input type="button" class="cateBtn tourUpdate" name="tourUpdate" value="수정">
											</div>
        								</table>
										<hr>`);
						// 클릭 이벤트 추가
